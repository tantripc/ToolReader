...
window.onerror = function (msg, url, line, column, err) {
    ...
    document.querySelector(".app .error .error-description").innerHTML = "Please try reloading the page or using a different browser (Chrome or Firefox), and if the error still persists.";
...
let App = function (el) {
	...
	if (params != null) {
		var url_file = params.get('url');
        //console.log(url_file);
        if (url_file) {
            this.doOpenBook(url_file);
        }
	}
    ...
    try {
        ...
        this.qs(".bar .loc").addEventListener("click", event => {
            try {
                let answer = prompt(`Nhập trang cần đến (tối đa ${this.state.book.locations.length()})?`, this.state.rendition.currentLocation().start.location);
                if (!answer) return;
                answer = answer.trim();
                if (answer == "") return;
...
App.prototype.doOpenBook = function (url) {
...
App.prototype.fatal = function (msg, err, usersFault) {
    ...
    document.querySelector(".app .error").classList.remove("hidden");
    document.querySelector(".app .error .error-title").innerHTML = "Error";
    document.querySelector(".app .error .error-description").innerHTML = usersFault ? "" : "Please try reloading the page or using a different browser, and if the error still persists";
...
App.prototype.onRenditionRelocatedUpdateIndicators = function (event) {
    try {
        if (this.getChipActive("progress") == "bar") {
            ...
            for (let i = 0, last = -1; i < this.state.book.locations.length(); i++) {
                try {
                    ...
                    marker.style.left = `<span class="toc"><b>${this.state.book.locations.percentageFromLocation(i) * 100}%</b></span>`;
        ...
        if (this.getChipActive("progress") == "none") {
            ...
            stxt = `<span class="toc"><b>${event.start.location}</b>/${this.state.book.locations.length()}</span>`
            ...
            stxt = (event.start.percentage > 0 && event.start.percentage < 1) ? `<span class="toc"><b>${Math.round(event.start.percentage * 1000) / 10}%</b></span>` : "";
        }
